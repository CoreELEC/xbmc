...
{
  struct vpp_pq_ctrl_s vpp_pq_ctrl = { 0 };
  struct pq_ctrl_s pq_ctrl = { 0 };
  ...
  {
    CLog::Log(LOGERROR, "CAMLCodec::Enable_vadj1 - cannot open amvecm driver /dev/amvecm: %s", strerror(errno));
    return false;
  }

  if (amvecm->IOControl(AMVECM_IOC_G_PQ_CTRL, &vpp_pq_ctrl) < 0)
  {
    CLog::Log(LOGERROR, "CAMLCodec::Enable_vadj1 - AMVECM_IOC_G_PQ_CTRL failed: %s", strerror(errno));
    return false;
  }
  ...
  if (pq_ctrl.vadj1_en != 1)
  {
    ...
    {
      CLog::Log(LOGERROR, "CAMLCodec::Enable_vadj1 - AMVECM_IOC_S_PQ_CTRL failed: %s", strerror(errno));
      return false;
    }
...
bool CAMLCodec::AddData(uint8_t *pData, size_t iSize, double dts, double pts)
{
  ...
  m_dll->codec_get_vbuf_state(&am_private->vcodec, &bs);
  if (iSize > 0)
    CLog::Log(LOGDEBUG, LOGVIDEO, "CAMLCodec::AddData: dl:%d sum:%u sz:%u dts:%0.3lf pts:%0.3lf", bs.data_len, m_frameSizeSum,
