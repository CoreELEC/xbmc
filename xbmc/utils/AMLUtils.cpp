...
bool aml_display_support_dv()
{
  ...
  if (support_dv == -1)
  {
    ...
    if (dv_cap.Exists())
    {
      valstr = dv_cap.Get<std::string>().value_or("");
...
int aml_display_get_dv_cap()
{
  ...
  if (dv_cap == 0)
  {
    ...
    {
      valstr = dv_cap_sys.Get<std::string>().value_or("");
...
int aml_axis_value(AML_DISPLAY_AXIS_PARAM param)
{
  ...
  if (display_axis.Exists())
    axis = display_axis.Get<std::string>().value_or("");
...
bool aml_get_native_resolution(RESOLUTION_INFO *res)
{
  ...
  if (display_mode.Exists())
    mode = display_mode.Get<std::string>().value_or("");
  ...
  if (aml_has_frac_rate_policy())
  {
    ...
    if (frac_rate_policy.Exists())
      fractional_rate = frac_rate_policy.Get<int>().value_or(0);
...
bool aml_probe_resolutions(std::vector<RESOLUTION_INFO> &resolutions)
{
  ...
  if (!user_dcapfile.Exists())
  {
    ...
    if (dcapfile.Exists())
      valstr = dcapfile.Get<std::string>().value_or("");
    else
      return false;
    ...
    {
      addstr = custom_mode.Get<std::string>().value_or("");
      valstr += "\n" + addstr;
    }
    ...
    {
      addstr = user_daddfile.Get<std::string>().value_or("");
      valstr += "\n" + addstr;
    }
  }
  else
    valstr = user_dcapfile.Get<std::string>().value_or("");
...
bool aml_read_reg(const std::string &reg, uint32_t &reg_val)
{
  ...
  {
    paddr.Set(reg);
    std::string val = paddr.Get<std::string>().value_or("");
